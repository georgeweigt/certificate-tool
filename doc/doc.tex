...
Let $s$ be the signature of $P$ and let $k$ be the public
key of $Q$.
...
For example, the following unencrypted signature is for RSA 2048 and hash digest SHA256.
(Numerals are in hexadecimal.)

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
{\footnotesize\tt 00} & {\footnotesize\tt 01} &
{\footnotesize\tt ff $\cdots$ ff} & {\footnotesize\tt 00} &
{\footnotesize\tt 30 31 30 0d 06 09 60 86 48 01 65 03 04 02 01 05 00 04 20} & HASH\\
\hline
\end{tabular}
\end{center}
...
In the actual certificate $P$, $s$ is encrypted using $Q$'s private key.
To verify that $P$ is signed by $Q$, $s$ is decrypted using $Q$'s public key $k$.
Then if HASH matches a digest of $P$, the signing of $P$ by $Q$ is verified.
