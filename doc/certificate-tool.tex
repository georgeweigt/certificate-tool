...
\noindent
Encoded integers may have an extra leading 00 byte due to encoding rules.
The rule adds the 00 byte if the first byte of the unencoded integer is greater than 127.
...
\noindent
Bit string data is the byte sequence $(04\mid X\mid Y)$ where both $X$ and $Y$ are 32 bytes long.
...
\subsection*{rime256v1 signature encoding}
...
\noindent
Bit string data is the byte sequence $(04\mid X\mid Y)$ where both $X$ and $Y$ are 48 bytes long.
